...
#
FREEBSD_BUILDWORLD_EXTRA_ARGS="-j$((`sysctl -n hw.ncpu` * 2))"
# FREEBSD_INSTALLWORLD_EXTRA_ARGS=""
FREEBSD_BUILDKERNEL_EXTRA_ARGS="-j$((`sysctl -n hw.ncpu` * 2))"
...
beaglebone_multiinstall_partition_image ( ) {
    local FAT_SIZE
    local PARTITION_SIZE
    local NUM_INSTALLS=2
    local LABEL_PREFIX
    ...
    case $FAT_SIZE in
        *k) FAT_SIZE=$(( ${FAT_SIZE%?} * 1000 ))
            ;;
        *m) FAT_SIZE=$(( ${FAT_SIZE%?} * 1000 * 1000 ))
            ;;
        *g) FAT_SIZE=$(( ${FAT_SIZE%?} * 1000 * 1000 * 1000 ))
            ;;
    esac

    # Derate the space we think we have by 5% (because, storage details), then divide it equally
    PARTITION_SIZE="$(( (${IMAGE_SIZE} - ${FAT_SIZE}) * 95 / 100 / ${NUM_INSTALLS} / 1000000 ))m"
...
strategy_add $PHASE_PARTITION_LWW beaglebone_multiinstall_partition_image
